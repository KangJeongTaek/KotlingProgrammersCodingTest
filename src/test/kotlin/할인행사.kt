...
//    예를 들어, 정현이가 원하는 제품이 바나나 3개, 사과 2개, 쌀 2개, 돼지고기 2개, 냄비 1개이며,
    //    XYZ 마트에서 14일간 회원을 대상으로 할인하는 제품이 날짜 순서대로 치킨, 사과, 사과, 바나나, 쌀, 사과, 돼지고기,
...
//    정현이가 원하는 제품을 나타내는 문자열 배열 want와 정현이가 원하는 제품의 수량을 나타내는 정수 배열 number,
    ...
    //    회원등록시 정현이가 원하는 제품을 모두 할인 받을 수 있는 회원등록 날짜의 총 일수를
    ...
    val discount = arrayOf("chicken", "apple",
        "apple", "banana", "rice",
        "apple", "pork", "banana", "pork",
        "rice", "pot", "banana", "apple", "banana")
    val result = 3
    @Test
    fun solution(){
        val wantCount = want.zip(number.toList()).toMap() // zip은 Pair을 반환하므로 map으로 변환한다.
        var answer = 0

        for(i in 0..discount.size - 10){
            val disCountSlice = discount.slice(i until i + 10).groupingBy { it }.eachCount()
            if(wantCount.all { (want,count) -> disCountSlice.getOrDefault(want,0) >= count }) answer ++
        }
